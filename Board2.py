...
class Board2:
    ...
    def __init__(self, randomize=False):
        ...
        # 先手Aのボード
        self.board_a = np.zeros((2, 2, 2), dtype=np.uint8)
        # 後手Bのボード
        self.board_b = np.zeros((2, 2, 2), dtype=np.uint8)
        # 勝ちパターンのリスト
        ...
        for i in range(2):
            for j in range(2):
                self.lines.append([(i, j, k) for k in range(2)])  # x-y平面
                self.lines.append([(i, k, j) for k in range(2)])  # x-z平面
                self.lines.append([(k, i, j) for k in range(2)])  # y-z平面
        # 面ではない対角線
        self.lines.extend([
            [(i, i, i) for i in range(2)],
            [(i, i, 1 - i) for i in range(2)],
            [(i, 1 - i, i) for i in range(2)],
            [(1 - i, i, i) for i in range(2)],
        ])
        # 面の上の対角線
        for i in range(2):
            self.lines.extend([
                [(i, j, j) for j in range(2)],
                [(i, j, 1 - j) for j in range(2)],
                [(j, i, j) for j in range(2)],
                [(j, i, 1 - j) for j in range(2)],
                [(j, j, i) for j in range(2)],
                [(j, 1 - j, i) for j in range(2)],
            ])
        if len(self.lines) != 28:
